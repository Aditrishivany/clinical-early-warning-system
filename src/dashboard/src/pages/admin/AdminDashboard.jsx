// File: src/dashboard/src/pages/admin/AdminDashboard.jsx
  ...
  const fetchAll = async () => {
    try {
      const [s, h, a] = await Promise.all([
        ...
        fetch('http://127.0.0.1:8000/api/v1/dashboard/high-risk').then(r => r.json()),
        fetch('http://127.0.0.1:8000/api/v1/dashboard/alerts').then(r => r.json()),
      ]);
      setStats(s);
    ...
    <div className="animate-fade">
      ...
      <div style={{
        ...
        <div style={{ position:'relative' }}>
          <p style={{ margin:'0 0 2px', fontSize:'12px', color:'rgba(212,175,55,0.8)', fontWeight:'600', letterSpacing:'0.5px' }}>
            {greeting} ·
      ...
      {/* Stats */}
      <div className="stagger" style={{ display:'flex', gap:'14px', marginBottom:'24px', flexWrap:'wrap' }}>
        <StatCard title="Total Patients" value={stats?.total_patients} icon="👥" color="#1e40af" subtitle="Currently monitored" delay={0.05} />
        <StatCard title="High Risk"      value={stats?.high_risk}      icon="🔴" color="#dc2626" subtitle="Immediate attention"  delay={0.10} />
        <StatCard title="Medium Risk"    value={stats?.medium_risk}    icon="🟡" color="#d97706" subtitle="Elevated monitoring"  delay={0.15} />
        <StatCard title="Low Risk"       value={stats?.low_risk}       icon="🟢" color="#059669" subtitle="Stable condition"     delay={0.20} />
        <StatCard title="Active Alerts"  value={stats?.active_alerts}  icon="🚨" color="#7c3aed" subtitle="Require action"      delay={0.25} />
        <StatCard title="Vitals Recorded" value={stats?.total_readings} icon="📊" color="#0891b2" subtitle="Total readings"      delay={0.30} />
      </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {highRisk.filter(p => p.risk_level === 2).map((p, i) => (
                <div key={i} style={{
                  ...
                  borderRadius:'var(--radius-md)', gap:'14px',
      ...
      <div className="card" style={{ padding:'24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:'10px' }}>
          {[
            { name:'ML Model',      status:'Online',  color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', icon:'🧠' },
            { name:'AI Agents',     status:'Online',  color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', icon:'🤖' },
            { name:'RAG System',    status:'Online',  color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', icon:'📚' },
            { name:'Database',      status:'Online',  color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', icon:'🗄️' },
            { name:'API Server',    status:'Online',  color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', icon:'⚡' },
            { name:'Azure SQL',     status:'Pending', color:'#d97706', bg:'#fffbeb', border:'#fde68a', icon:'☁️' },
            { name:'Azure OpenAI',  status:'Pending', color:'#d97706', bg:'#fffbeb', border:'#fde68a', icon:'🤖' },
            { name:'Data Factory',  status:'Pending', color:'#d97706', bg:'#fffbeb', border:'#fde68a', icon:'🔄' },
          ].map((sys, i) => (
            <div key={sys.name} style={{
              ...
              <div>
                ...
                <p style={{ margin:0, fontSize:'10px', color:sys.color, fontWeight:'700' }}>
                  ● {sys.status}
                </p>
              </div>
            </div>
